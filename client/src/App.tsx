...
import { Search, Heart, Calendar, User, Menu, X, Bell, ShieldCheck, LogOut, Sparkles, ChevronRight } from "lucide-react";
...
export default function App() {
  ...
  React.useEffect(() => {
    ...
    if (savedUser) {
      ...
      if (savedUser.role === 'client') setCurrentScreen('curatedVendors');
      else if (savedUser.role === 'vendor') setCurrentScreen('vendorDashboard');
  ...
  const handleLogin = (role: string) => {
    setUserRole(role);
    if (role === 'client') setCurrentScreen('curatedVendors');
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case 'onboarding':
        return <OnboardingScreen onNext={() => setCurrentScreen('curatedVendors')} />;
      case 'curatedVendors':
        return <CuratedVendorsScreen onVendorClick={(id) => {
          setSelectedVendorId(id);
          setCurrentScreen('vendorProfile');
        }} />;
      case 'vendorProfile':
        return <VendorProfileScreen
          onBack={() => setCurrentScreen('curatedVendors')}
          onBook={() => setCurrentScreen('booking')}
          vendorId={selectedVendorId}
        />;
      case 'booking':
        return <BookingScreen
          onBack={() => setCurrentScreen('vendorProfile')}
          onNext={() => setCurrentScreen('invoice')}
          vendorId={selectedVendorId}
        />;
      case 'invoice':
        return <InvoiceScreen onBack={() => setCurrentScreen('checklist')} />;
      case 'checklist':
  ...
  const displayName = () => {
    if (!user) return '';
    if (userRole === 'client') return user.name;
    if (userRole === 'vendor') return user.name;
    return 'Admin Portal';
  };
